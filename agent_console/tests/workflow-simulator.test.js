...
assert.match(result.steps[0].internalPrompt, /редакционный продюсер/);
...
assert.doesNotMatch(result.finalTelegramText, /Для BAURUM здесь важен/);
assert.doesNotMatch(result.finalTelegramText, /качество различения/);
assert.doesNotMatch(result.finalTelegramText, /что мы действительно знаем из источника/);
assert.doesNotMatch(result.finalTelegramText, /Хороший материал/);
assert.doesNotMatch(result.finalTelegramText, /Пост о том/);
