...
function firstMatching(payload, pattern) {
  for (const [key, value] of Object.entries(payload)) {
    const cleaned = clean(value);
    if (pattern.test(key) && cleaned) return cleaned;
  }
  return "";
}
...
function specialOrderData(payload) {
  return {
    ...
    comment: first(payload, ["Your additional comment", "Comment", "Комментарий"]) ||
      firstMatching(payload, /comment|additional|коммент/i)
  };
}
...
module.exports._test = { requestType, language, first, firstMatching, specialOrderData };
const HUBSPOT_BASE = "https://api.hubapi.com";
